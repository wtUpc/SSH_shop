...
//使用spring对Hibernate的模版 HibernateTemplate
	...
	public User queryUser(String username, String name, String sex,
		...
		//String hql = "from User";
		return (User)hibernateTemplate.getSessionFactory().openSession().createQuery(
				"from User u where u.username=? and u.name=? and u.sex=? and u.address=? and u.phone=?").setParameter(
						0, username).setParameter(1, name).setParameter(2, sex).setParameter(3, address).setParameter(4,phone).uniqueResult();
